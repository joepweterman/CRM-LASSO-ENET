crm_enet <- function (formula, data, maxiter = 100, tolerance = 0.01, outlyingness.factor = 1,
  ...
  if (any(wr < 1)) {
    ...
    suppressWarnings(
      outlvars <- lapply(outliers, function (x) {spadimo_elasticnet(data = datamc[, 2:nCol],
                                                               weights = wr, obs = x,
  ...
  while (difference > tolerance & loop_counter <= maxiter & !early_stop) {
    ...
    if (any(wr < 1)) {
      outliers <- as.list(which(wr < 1)) # casewise outliers in X-space (i.e. cases with wr[i] < 1)
      suppressWarnings(
        outlvars <- lapply(outliers, function (x) spadimo_elasticnet(data = datamc_imputed[, 2:nCol],
                                                                weights = wr, obs = x,
                                                                control = list(scaleFun  = Qn,
                                                                               nlatent   = 1,
                                                                               etas      = spadieta,
