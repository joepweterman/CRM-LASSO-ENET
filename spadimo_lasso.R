spadimo_l <- function(data, weights, obs,
                                   ...
                                   plot      = TRUE))
{
  ...
  #                     nlatent: integer number of latent variables for sparse PLS regression (via Lasso) (default is 1)
...
spadimo_lasso <- function(data, weights, obs, control = list(etas = NULL)) {
  x <- as.matrix(data)
  n <- nrow(x)
  p <- ncol(x)
  w <- weights
  ...
  for (i in 1:length(etas)) {
    lasso_model <- glmnet(x, w, family = "gaussian", alpha = 1, lambda = etas[i])
    outlvars <- which(coef(lasso_model) != 0)[-1]
    a <- coef(lasso_model)[-1]
