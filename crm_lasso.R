crm_lasso <- function (formula, data, maxiter = 100, tolerance = 0.01, outlyingness.factor = 1,
  ...
  if (any(wr < 1)) {
    ...
    suppressWarnings(
      outlvars <- lapply(outliers, function (x) {spadimo_lasso(data = datamc[, 2:nCol],
                                                         weights = wr, obs = x,
  ...
  while (difference > tolerance & loop_counter <= maxiter & !early_stop) {
    ...
    if (any(wr < 1)) {
      outliers <- as.list(which(wr < 1)) # casewise outliers in X-space (i.e. cases with wr[i] < 1)
      suppressWarnings(
        outlvars <- lapply(outliers, function (x) spadimo_lasso(data = datamc_imputed[, 2:nCol],
                                                          weights = wr, obs = x,
                                                          control = list(scaleFun  = Qn,
                                                                         nlatent   = 1,
                                                                         etas      = spadieta,
    ...
    ## Update residuals and derive case weights with Hampel weight function
    fitted_values <- datamc_imputed[, -1] %*% matrix(betas, ncol = 1) + intercept
    r <- datamc_imputed[, 1] - fitted_values
    r <- scaleResidualsByMAD(r)
    wr <- HampelWeightFunction(r, q1 = qnorm(0.95), q2 = qnorm(0.975), q3 = qnorm(0.999))
