...
for (n_value in n_seq) {

  n_index <- which(n_seq == n_value)
  cat(paste("\n*", n_index, "out of", length(n_seq), "========================================\n"))
  ...
  for (j_sim in 1:n_sims) {
    ...
    X <- mvrnorm(n = n_value, mu = mu, Sigma = Sigma)

    slopes <- rnorm(n = p, mean = 0, sd = 1)
    slopes <- 10 * slopes / sqrt(sum(slopes^2))
    intercept <- 10
    noise <- rnorm(n = n_value, mean = 0, sd = 0.5)
    ...
    case_outliers <- sample(n_value, size = n_value * pct_case_out)
    outliers_mat_flag <- matrix(FALSE, nrow = n_value, ncol = p)
    ...
    cat(paste0(" - ", round(100 * (j_sim + (n_index - 1) * n_sims) / (n_sims * length(n_seq)), 2),
  ...
  results_MAE[[n_index]]       <- results_MAE_k
  results_MSEP[[n_index]]      <- results_MSEP_k
  results_RMSEI[[n_index]]     <- results_RMSEI_k
  results_time[[n_index]]      <- results_time_k
...
# Study results -----------------------------------------------------------------------------------
results_MAE       <- cbind.data.frame(do.call(rbind, results_MAE),       n = sort(rep(n_seq, n_sims)))
results_MSEP      <- cbind.data.frame(do.call(rbind, results_MSEP),      n = sort(rep(n_seq, n_sims)))
results_RMSEI     <- cbind.data.frame(do.call(rbind, results_RMSEI),     n = sort(rep(n_seq, n_sims)))
results_time      <- cbind.data.frame(do.call(rbind, results_time),      n = sort(rep(n_seq, n_sims)))

results_MAE_mean       <- aggregate(. ~ n, data = results_MAE,       FUN = mean)
results_MSEP_mean      <- aggregate(. ~ n, data = results_MSEP,      FUN = mean)
results_RMSEI_mean     <- aggregate(. ~ n, data = results_RMSEI,     FUN = mean)
results_time_mean      <- aggregate(. ~ n, data = results_time,      FUN = mean)


# Mean Absolute Error - - - - - - - - - - - - - -
plot(results_MAE_mean$n, results_MAE_mean$`CRM`, type = "l", xlab = "n", ylab = "Average MAE",
     ylim = range(results_MAE_mean[, -1]), las = 1, cex.axis = 1.2, cex.lab = 1.3, lwd = 2)
lines(results_MAE_mean$n, results_MAE_mean$`CRM-LASSO`,      lwd = 2, lty = 2)
lines(results_MAE_mean$n, results_MAE_mean$`CRM-ElasticNet`,  lwd = 2, lty = 3)
legend("topright", legend = names(results_MAE_mean)[-1], lty = 1:3, lwd = 2)


# Mean Squared Error of Prediction - - - - - - - -
plot(results_MSEP_mean$n, results_MSEP_mean$`CRM`, type = "l", xlab = "n", ylab = "Average MSEP",
     ylim = range(results_MSEP_mean[, -1]), las = 1, cex.axis = 1.2, cex.lab = 1.3, lwd = 2)
lines(results_MSEP_mean$n, results_MSEP_mean$`CRM-LASSO`,      lwd = 2, lty = 2)
lines(results_MSEP_mean$n, results_MSEP_mean$`CRM-ElasticNet`,  lwd = 2, lty = 3)
legend("bottomright", legend = names(results_MSEP_mean)[-1], lty = 1:3, lwd = 2)


# Root Mean Squared Error of Imputation - - - - -
plot(results_RMSEI_mean$n, results_RMSEI_mean$`CRM`, type = "l", xlab = "n", ylab = "Average RMSEI",
