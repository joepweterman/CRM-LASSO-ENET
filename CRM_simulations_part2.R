...
n_sims <- 10           # number of simulations for each value of k
...
for (k_value in k_seq) {
  ...
  for (j_sim in 1:n_sims) {
    ...
    cat(paste0(" - ", round(100 * (j_sim + (k_index - 1) * n_sims) / (n_sims * length(k_seq)), 2),
...
# Study results -----------------------------------------------------------------------------------
results_MAE       <- cbind.data.frame(do.call(rbind, results_MAE),       k = sort(rep(k_seq, n_sims)))
results_MSEP      <- cbind.data.frame(do.call(rbind, results_MSEP),      k = sort(rep(k_seq, n_sims)))
results_RMSEI     <- cbind.data.frame(do.call(rbind, results_RMSEI),     k = sort(rep(k_seq, n_sims)))
results_time      <- cbind.data.frame(do.call(rbind, results_time),      k = sort(rep(k_seq, n_sims)))
